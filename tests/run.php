...
foreach ($articles as $article) {
    ...
    check(!empty($article['reviewer']) && stripos($article['reviewer'], 'ergänzen') === false && stripos($article['reviewer'], 'vor Veröffentlichung') === false, 'Sichtbarer Prüfstatus ist final: ' . $article['slug']);
}
...
foreach ($seoPages as $key => $seo) {
    if (str_contains((string) ($seo['robots'] ?? ''), 'noindex') || $key === '404') {
        continue;
    }
    check(mb_strlen($seo['title']) >= 30 && mb_strlen($seo['title']) <= 65, 'Titel hat passende Länge: ' . $key);
    check(mb_strlen($seo['description']) >= 120 && mb_strlen($seo['description']) <= 170, 'Beschreibung hat passende Länge: ' . $key);
}
