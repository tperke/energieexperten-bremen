...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
    ...
    if ($errors === []) {
        $sent = send_portal_mail('Kontaktanfrage über Energieexperten Bremen', ['Thema: ' . $subject, 'Name: ' . $name, 'E Mail: ' . $email, '', 'Nachricht:', $message], $email);
        if ($sent) {
...
<main id="main-content">
    ...
    <section class="section form-section"><div class="wrap form-layout"><div class="form-card">
        <?php if (!mail_is_configured()): ?><div class="notice warning" role="status"><strong>SMTP noch gesperrt:</strong> Tragen Sie vor der produktiven Nutzung das E-Mail-Passwort in der Serverkonfiguration ein. Das Formular speichert in diesem Zustand keine Nachricht.</div><?php endif; ?>
        ...
        <form method="post" action="<?= e(site_url('kontakt/')) ?>" class="secure-form">
            ...
            <div class="privacy-box"><h2>Datenschutzhinweis</h2><p>neu-protec Mediendesign verarbeitet Ihre Angaben ausschließlich zur Bearbeitung dieser Nachricht. Die Übermittlung in das IONOS-Postfach erfolgt verschlüsselt. Anfragen werden grundsätzlich spätestens sechs Monate nach Abschluss gelöscht, sofern keine gesetzlichen Pflichten oder Rechtsansprüche entgegenstehen.</p><label class="checkbox"><input type="checkbox" name="privacy_acknowledged" value="1" required<?= checked(isset($old['privacy_acknowledged'])) ?>><span>Ich habe den Datenschutzhinweis und die <a href="<?= e(site_url('datenschutz/')) ?>">Datenschutzerklärung</a> gelesen. <span aria-hidden="true">*</span></span></label></div>
